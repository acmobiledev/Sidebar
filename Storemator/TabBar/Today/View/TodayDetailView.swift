...
//MARK: - Properties
    let containerView: UIView = {
        let aView = UIView()
        aView.frame = CGRect(x: 0, y: 0, width: getCurrentWindow().bounds.size.width, height: 400)
    ...
    lazy var detailTextView: UITextView = {
        let aTV = UITextView()
        aTV.font = .boldSystemFont(ofSize: 15)
        ...
        aTV.frame = CGRect(x: 20, y: self.containerView.frame.height + 40.0, width: getCurrentWindow().bounds.size.width - 40, height: aTV.text.calculateHeightWith(width: getCurrentWindow().bounds.size.width - 40, font: UIFont.boldSystemFont(ofSize: 16)))
        return aTV
    }()
...
//        containerView.anchorView(top: self.topAnchor, left: self.leftAnchor, bottom: nil, right: self.rightAnchor, topPadding: 0, leftPadding: 0, bottomPadding: 0, rightPadding: 0, width: self.bounds.width, height: 400)
//
...
//        detailImageView.anchorView(top: self.containerView.topAnchor, left: self.containerView.leftAnchor, bottom: self.containerView.bottomAnchor, right: self.containerView.rightAnchor, topPadding: 0, leftPadding: 0, bottomPadding: 0, rightPadding: 0, width: 0, height: 0)
        detailImageView.frame = containerView.bounds
        
        self.addSubview(detailTextView)
//        detailTextView.anchorView(top: self.containerView.bottomAnchor, left: self.leftAnchor, bottom: nil, right: self.rightAnchor, topPadding: 0, leftPadding: 20, bottomPadding: 0, rightPadding: 20, width: self.bounds.width, height: 500)
